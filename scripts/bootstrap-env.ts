...
const logSuccess = (message: string) => log(colors.green, `✅ ${message}`);
const logWarning = (message: string) => log(colors.yellow, `⚠️  ${message}`);
const logError = (message: string) => log(colors.red, `❌ ${message}`);
const logInfo = (message: string) => log(colors.blue, `ℹ️  ${message}`);
const logBold = (message: string) => log(colors.bold, message);
...
function checkEnvFile(config: EnvConfig): boolean {
